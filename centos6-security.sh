...
#密码复杂度设置
sed -i 's#password    requisite     pam_cracklib.so try_first_pass retry=3 type=#password requisite pam_cracklib.so retry=3 difok=3 minlen=10 ucredit=-1 lcredit=-2 dcredit=-1 ocredit=-1 enforce_for_root#' /etc/pam.d/system-auth
